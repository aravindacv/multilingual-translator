...
// // // ── Script validators ──────────────────────────────────────────────────────
// // function isJapanese(t)  { for(const c of t){const p=c.codePointAt(0);if((p>=0x3040&&p<=0x30FF)||(p>=0x4E00&&p<=0x9FFF))return true;}return false; }
// // function isArabic(t)    { for(const c of t){const p=c.codePointAt(0);if(p>=0x0600&&p<=0x06FF)return true;}return false; }
// // function isKannada(t)   { for(const c of t){const p=c.codePointAt(0);if(p>=0x0C80&&p<=0x0CFF)return true;}return false; }
// // function isMalayalam(t) { for(const c of t){const p=c.codePointAt(0);if(p>=0x0D00&&p<=0x0D7F)return true;}return false; }
// // function isHindi(t)     { for(const c of t){const p=c.codePointAt(0);if(p>=0x0900&&p<=0x097F)return true;}return false; }

// // const SCRIPT_VALIDATORS = { ja:isJapanese, ar:isArabic, kn:isKannada, ml:isMalayalam, hi:isHindi };

// // // ── Fetch with script validation + retry ───────────────────────────────────
// // async function fetchTranslation(text, targetLang, attempt=1) {
// //   const controller = new AbortController();
// //   const timeout    = setTimeout(() => controller.abort(), CONFIG.REQUEST_TIMEOUT);
// //   try {
...
// // async function translateAll(text) {
// //   const [ja, ar, kn, ml, hi] = await Promise.allSettled([
// //     fetchTranslation(text, CONFIG.TARGET_LANGS.japanese),
// //     fetchTranslation(text, CONFIG.TARGET_LANGS.arabic),
// //     fetchTranslation(text, CONFIG.TARGET_LANGS.kannada),
// //     fetchTranslation(text, CONFIG.TARGET_LANGS.malayalam),
// //     fetchTranslation(text, CONFIG.TARGET_LANGS.hindi),
// //   ]);
// //   return {
// //     japanese:  ja.status==="fulfilled" ? ja.value : null,
// //     arabic:    ar.status==="fulfilled" ? ar.value : null,
// //     kannada:   kn.status==="fulfilled" ? kn.value : null,
// //     malayalam: ml.status==="fulfilled" ? ml.value : null,
// //     hindi:     hi.status==="fulfilled" ? hi.value : null,
// //   };
// // }
...
// ── Translate all 5 API languages in parallel ─────────────────────────────
// Tulu is NOT included here — it comes from local TULU_DICT only
async function translateAll(text) {
  const [ja, ar, kn, ml, hi] = await Promise.allSettled([
    fetchTranslation(text, CONFIG.TARGET_LANGS.japanese),
    fetchTranslation(text, CONFIG.TARGET_LANGS.arabic),
  ...
  return {
    ...
    malayalam: ml.status==="fulfilled" ? ml.value : null,
    hindi:     hi.status==="fulfilled" ? hi.value : null,
    // Tulu comes from lookupTulu() in tulu_dictionary.js — not from API
    tulu:      null,
  };
}